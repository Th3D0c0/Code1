namespace DiceRoll{
    ...
    export function selectDieType(): string{
        ...
        let userInput: number = parseInt(prompt("What Die to choose: (1):D4, (2):D6, (3):D8, (4):D10, (5):D12, (6): D20, (0): Continue") || "-1");
    ...
    export function sumValues(_currentDieData: [], _maxDieData: [], _dieType: any, _rollAmount: number): void{
        for (let i = 0; i < _currentDieData[_dieType]; i++){
        let localDieStorage: number = getRandomInt(1,maxDieData[_dieType]);
        //Sum up Values
        rollSum += localDieStorage;
        //Stash the current Value
        numberStash[_dieType].push(localDieStorage);
        }
    }


    export function startSimulation(_currentDieData:[], _maxDieData: []): void{
    ...
    export function selectRollAmount(): void{
       let globalRollAmount: number = Number(prompt("How many Rolls do you want"));
       for (let i: number = 0; i < globalRollAmount; i++){
